...
/// The frequency of the clock that is internal to the TMC5160.
///
/// Sometimes referred to as `fclk` in the datasheet.
pub const INTERNAL_CLOCK_HZ: f32 = 12_000_000.0;

/// A serial reader, for reading responses via the TMC5160's UART interface.
///
...
#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd, Format)]
...
#[repr(C)]
#[derive(Copy, Clone, Debug, Eq, Hash, PartialEq, Format)]
...
pub fn send_read_request<R, U, X>(
    slave_addr: u8,
    uart_tx: &mut U,
    uart_rx: &mut X,
) -> Result<(), U::Error>
    ...
    X: Read<u8>,
{
    ...
    //info!("sending read requst:{}", req);
    for data in req.bytes() {
        block!(uart_tx.write(*data))?;
    }
    block!(uart_tx.flush())?;
    let mut counter = req.bytes().len();
    while counter > 0 {
        match uart_rx.read() {
            Ok(_) => counter -= 1,
...
pub fn send_write_request<R, U, X>(
    slave_addr: u8,
    reg: R,
    uart_tx: &mut U,
    uart_rx: &mut X,
