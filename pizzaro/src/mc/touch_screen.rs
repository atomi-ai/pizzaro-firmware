...
impl TouchScreenEnum {
    pub fn new_button(screen_id: u16, object_id: u16, clicked: bool) -> Self {
        Self::Button { screen_id, object_id, clicked }
    }
    ...
    pub fn to_update(&self, buf: &mut [u8]) -> Result<usize, TouchScreenError> {
        ...
        let size = match self {
            ...
            Self::Text {
                ...
                buf[3..5].copy_from_slice(&screen_id.to_be_bytes());
                buf[5..7].copy_from_slice(&object_id.to_be_bytes());
                let content_len = content.iter().position(|&x| x == 0).unwrap_or(content.len());
    ...
    pub fn to_read(&self, buf: &mut [u8]) -> Result<usize, TouchScreenError> {
        ...
        let (screen_id, object_id) = match self {
            Self::Button { screen_id, object_id, clicked: _clicked } => (screen_id, object_id),
            Self::Slider { screen_id, object_id, value: _value } => (screen_id, object_id),
            Self::Text {
                // 更新文本框
                screen_id,
                object_id,
    ...
    pub fn parse(frame: &[u8]) -> Result<Self, TouchScreenError> {
        ...
        let screen_id = u16::from_be_bytes(
            frame[3..5].try_into().map_err(|_| TouchScreenError::FromBeBytesError)?,
        );
        let object_id = u16::from_be_bytes(
            frame[5..7].try_into().map_err(|_| TouchScreenError::FromBeBytesError)?,
        );
        ...
        match control_type {
            0x13 => {
                ...
                let value = u32::from_be_bytes(
                    frame[8..12].try_into().map_err(|_| TouchScreenError::FromBeBytesError)?,
                );
                Ok(Self::Slider { screen_id, object_id, value })
            }
            0x10 => {
                // 按钮控件
                let value = frame[9];
                Ok(Self::Button { screen_id, object_id, clicked: value == 0x1 })
            }
            0x11 => {
                // 文本控件
