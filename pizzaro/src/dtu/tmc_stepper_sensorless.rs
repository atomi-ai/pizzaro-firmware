...
pub const FAST_SPEED: u32 = 1000; // steps / second
...
impl<OP1, OP2, OP3, D, U, X> TmcStepper<OP1, OP2, OP3, D, U, X>
where
    OP1: StatefulOutputPin,
    OP2: OutputPin,
    OP3: OutputPin,
    ...
    X: Read<u8>,
{
    pub fn new(mut stepper: TmcDriver<OP1, OP2, OP3, D, U, X>) -> Self {
    ...
    pub async fn home(&mut self) -> Result<i32, AtomiError> {
        ...
        let steps = self.move_to_relative_internal(FAST_SPEED, -i32::MAX).await.map_err(|e| {
            self.state.pop();
            e
        })?;
    ...
    async fn move_to_relative_internal(
        ...
        for _i in 0..steps.abs() {
            if GLOBAL_DTU_STEPPER_STOP.load(Ordering::Relaxed) {
                return Err(AtomiError::DtuStopped);
            }
            // if i % 100 == 0 {
            //     // check status
            //     let sg_result = self.stepper.sg_result();
            //     info!("Got SG_RESULT: {}", Debug2Format(&sg_result));
            // }
            // let l = self.limit_left.is_high().unwrap_or(false);
            // let r = self.limit_right.is_high().unwrap_or(false);
            // info!(
