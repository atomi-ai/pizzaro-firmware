...
use crate::dtu::GLOBAL_DTU_LINEAR_STEPPER_STOP;
...
pub struct DtuLinearStepper<
    ...
    X: Read<u8>,
> {
    stepper: TmcStepper<OP1, OP2, OP3, D, U, X>,
    is_home: AtomicBool,
...
impl<IP1, IP2, OP1, OP2, OP3, D, U, X> DtuLinearStepper<IP1, IP2, OP1, OP2, OP3, D, U, X>
where
    IP1: InputPin,
    IP2: InputPin,
    OP1: StatefulOutputPin,
    ...
    X: Read<u8>,
{
    pub fn new(mut stepper: TmcStepper<OP1, OP2, OP3, D, U, X>, limit_left: IP1, limit_right: IP2) -> Self {
    ...
    async fn move_to_relative_internal(
        ...
        for i in 0..steps.abs() {
            if GLOBAL_DTU_LINEAR_STEPPER_STOP.load(Ordering::Relaxed) {
